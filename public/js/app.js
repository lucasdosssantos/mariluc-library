...
/* Menu de navegação mobile (hambúrguer) — acessível por teclado e mouse.
   Sem isso, em telas estreitas o menu principal ficava totalmente
   inacessível (apenas escondido em CSS, sem forma de reabrir). */
const navToggle = document.getElementById("navToggle");
const mainNav = document.getElementById("mainNav");
if (navToggle && mainNav) {
  const closeNav = () => { mainNav.classList.remove("open"); navToggle.setAttribute("aria-expanded", "false"); };
  const openNav = () => { mainNav.classList.add("open"); navToggle.setAttribute("aria-expanded", "true"); };
  ...
  document.addEventListener("click", e => {
    if (mainNav.classList.contains("open") && !mainNav.contains(e.target) && e.target !== navToggle) closeNav();
  });
  ...
  app.innerHTML = `
    <section class="hero">
      <div class="welcome-card">
        ...
        <img class="hero-logo" src="/img/logo.png" alt="" onerror="this.remove()">
...
async function acervoPage(params) {
  ...
  app.innerHTML = `
    <section class="section">
      <div class="section-title"><h2>Nosso Acervo</h2><p>Pesquise e filtre os livros da MariLuc Library.</p></div>
...
function showAuth(mode) {
  ...
  document.getElementById("authForm").innerHTML = mode === "login" ? `
    <form id="loginForm">
      ...
      <div class="field"><label for="senha">Senha</label><input class="input" id="senha" type="password" required></div>
    ...
    <form id="registerForm">
      <div class="form-grid">
        ...
        <div class="field"><label for="email">E-mail</label><input class="input" id="email" type="email" required></div>
        <div class="field"><label for="senha">Senha</label><input class="input" id="senha" type="password" minlength="6" required></div>
        <div class="field"><label for="bio">Bio</label><input class="input" id="bio" maxlength="120" placeholder="Conte um pouco sobre você"></div>
      </div>
